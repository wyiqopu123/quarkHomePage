$(function () {
	...
	if (!localStorage.getItem("setData")) {
		...
		var setData = { engines: "quark", wallpaper: "", logo: "", bookcolor: "black" };
	...
	var Storage = [];
	// 加载壁纸|LOGO|书签颜色
	Storage.setData = JSON.parse(localStorage.getItem("setData"));
	...
	// 加载首页书签内容
	Storage.bookMark = JSON.parse(localStorage.getItem("bookMark"));
	var html = '';
	...
	$(".bookmark").longPress(function () {
		if (!$(".addbook").length && !$(".delbook").length) {
			$(".bookmark").sortable({
				...
				onStart: evt => {
					//开始拖拽出发该函数
					this.startID = $(evt.item).index();
				},
				onEnd: evt => {
					//拖拽完毕之后发生该事件
					var startID = this.startID;
					var endID = $(evt.item).index();
					if (startID > endID) {
					...
					localStorage.setItem("bookMark", JSON.stringify(Storage.bookMark));
				}
			});
			...
			$(".delbook").click(function () {
				$(".bookmark").css("overflow", "visible");
				var dom = $(this).parent().parent(); // .list
				var index = dom.index();
			...
			if ($list.length < 20) {
				$(".bookmark").append('<div class="list addbook" data-url="addbook()"><div class="img"><svg viewBox="0 0 1024 1024"><path d="M736.1 480.2H543.8V287.9c0-17.6-14.4-32-32-32s-32 14.4-32 32v192.2H287.6c-17.6 0-32 14.4-32 32s14.4 32 32 32h192.2v192.2c0 17.6 14.4 32 32 32s32-14.4 32-32V544.2H736c17.6 0 32-14.4 32-32 0.1-17.6-14.3-32-31.9-32z" fill="#212121"></path></svg></div></div>');
			}
		}
	});
	// 绑定书签点击事件
	$(".bookmark").on('click', '.list', function (e) {
		e.stopPropagation();
	...
	// 添加书签页面
	function addbook() {
		$(document).click(); // 取消书签编辑状态
		$('#app').append(`<div class="addbook-shade">
			<div class="addbook-from">
				<div class="addbook-title">添加书签</div>
				<div class="addbook-content">
					<input type="text" class="addbook-input addbook-name" placeholder="标题" />
					<input type="text" class="addbook-input addbook-url" placeholder="地址" />
					<div id="addbook-upload">点击选择图标</div>
					<input type="file" id="addbook-pick" />
				</div>
				<div class="addbook-btn"><a class="addbook-close">取消</a><a class="addbook-ok">确定</a></div>
			</div>
		</div>`);
		...
		$(".addbook-ok").click(function () {
			...
			if (name.length && url.length) {
				if (!icon) {
					icon = textBecomeImg(name.substr(0, 1));
				}
				$(".addbook-close").click();
		...
		$(".addbook-shade").click(function (evt) {
			if (evt.target.className === "addbook-shade") $(".addbook-close").click();
		});
	}
	...
	function choice() {
		...
		// 动画完成后加载，防止过渡动画卡顿
		function aniEnd(evt) {
			if (evt.target !== this) { // 过滤掉子元素
				return;
			}
			$(".page-choice").off("transitionend", aniEnd);
				...
				success: function (location) {
					$.ajax({
						...
						success: function (data) {
							...
							$span = $('.choice-cut').find('.weather').find('span');
							$span.eq(0).text(Math.round(data.result.realtime.temperature));
							$span.eq(1).text(skycon[data.result.realtime.skycon].name);
							$span.eq(2).text(location.city.replace(/市$/, '') + " · 空气" + data.result.realtime.air_quality.description.chn);

							var color = skycon[data.result.realtime.skycon].color.split(',');
							$content = $('.choice-cut').find('.weather').find('.content');
							$content.css("background-image", "linear-gradient(to right bottom," + color[0] + "," + color[1] + ")");
							$content.append('<div class="' + skycon[data.result.realtime.skycon].icon + '"></div>');
						}
					});
				}
			});
		...
		$(".page-choice").on("transitionend", aniEnd);
	...
	$(".logo").click(() => {
		...
		$(".set-select").change(function () {
			var dom = $(this);
			var value = dom.parent().data("value");
			Storage.setData[value] = dom.val();
			localStorage.setItem("setData", JSON.stringify(Storage.setData));
			if (value === 'bookcolor') {
