...
var bookMark = {
		...
		_.$.bookmark.longPress(function () {
			if (_.status !== "editing") {
				...
				_.$.bookmark.sortable({
					...
					onEnd: function (evt) {
						_.callback({ event: "sort", start: evt.oldIndex, end: evt.newIndex });
					}
				});
	...
	del: function (index) {
		...
		var dom = _.$.bookmark.find('.list').eq(index); // .list
		...
		_.callback({ event: "del", index: index });
	},
	add: function (name, url, icon) {
		...
		_.callback({ event: "add", name: name, url: url, icon: icon });
	}
};

// 初始化首页书签模块
bookMark.init('.bookmark', function (evt) {
	if (evt.event === "sort") {
		var startID = evt.start,
			endID = evt.end;
		if (startID > endID) {
		...
		Storage.bookMark.push({ name: evt.name, url: evt.url, icon: evt.icon });
	} else if (evt.event === "del") {
		Storage.bookMark.splice(evt.index, 1);
	}
